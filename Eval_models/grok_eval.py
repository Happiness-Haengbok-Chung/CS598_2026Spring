...
try:
    ...
    print(f"통신 중 에러가 발생했어요: {e}")

def get_hallucination_label_grok(dialogue_text):
    # xAI 공식 엔드포인트
    ...
    payload = {
        "model": "grok-4-1-fast-reasoning", # 또는 사용 가능한 최신 모델명
        "messages": [
...
def run_relabeling(input_json_path, output_json_path):
    ...
    for item in tqdm(data, desc="Relabeling"):
        ...
        # GPT-4o Judge 호출
        new_label = get_hallucination_label_grok(dialogue_text)
...
output_path = 'DiaHalu_Bench_relabeled_Grok-4-1-fast-reasoning.json'
