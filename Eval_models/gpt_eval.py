...
try:
    ...
    # 가나다순(알파벳순)으로 정렬해서 보기 편하게 출력
    for model_name in sorted(model_list):
        print(f" - {model_name}")
        
    print(f"\n총 {len(model_list)}개의 모델이 검색되었습니다.")

except Exception as e:
    print(f"모델 목록을 가져오는 중 오류가 발생했어요: {e}")

def get_hallucination_label(dialogue_text):
    """
    GPT-4o를 사용하여 대화 속 할루시네이션 유형 번호를 추출합니다.
    """
    headers = {
    ...
    payload = {
        ...
        "messages": [
            ...
            {"role": "user", "content": f"Analyze this dialogue and provide the first error type number:\n\n{dialogue_text}"}
        ],
        "temperature": 0.0,
        "max_tokens": 5
    }
...
def run_relabeling(input_json_path, output_json_path):
    ...
    for item in tqdm(data, desc="Relabeling"):
        ...
        # GPT-4o Judge 호출
        new_label = get_hallucination_label(dialogue_text)
...
# --- 실행 ---
input_path = 'DiaHalu_Bench_Sampled.jsonl' # 원본 파일명으로 수정하세요
output_path = 'DiaHalu_Bench_relabeled_GPT-5.5.json'
